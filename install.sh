...
install_dokploy() {
    ...
    # check if something is running on port 70
    if ss -tulnp | grep ':70 ' >/dev/null; then
        echo "Error: something is already running on port 70" >&2
        exit 1
    fi
    ...
    #     --publish mode=host,published=70,target=70 \
